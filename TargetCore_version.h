...
//
//  3.1.0.0. The MINOR moved for the reason the versioning policy says it
//  moves: the covered surface GREW and nothing on it changed meaning. The
//  security revision added eight symbols to the flat C ABI - the per-class
//  link policy, the trust fence and the end-to-end waiver, with their
//  getters - and every one of them is new. A consumer built against 3.0.0
//  links against 3.1.0 unchanged, which is what a MINOR promises and the
//  whole of what it promises.
//
//  3.0.0.0 was the identity chosen for the first PUBLIC release, and that
//  number was set by the project rather than derived from this tree's own
//  release history: the development identities that preceded it here were
//  0.9.0 and then 0.10.0, and no binary carrying either of them was
//  published.
//
...
//  message does not report a mismatch, it parses garbage. Releasing a number
//  of this shape - 3.0.0 then 3.1.0 - neither freezes that image nor makes it
//  safe to change.
...
#define TARGETCORE_VERSION_COMMAS 3,1,0,0
...
#define TARGETCORE_VERSION_STRING "3.1.0.0"

//  Packed form, for a consumer that wants to compare rather than display.
//  0x03010000 is 3.1.0.0; the byte order is MAJOR, MINOR, PATCH, BUILD -- one
//  byte each.
#define TARGETCORE_VERSION_HEX    0x03010000
...
#define TARGETCORE_VERSION_STRINGW L"3.1.0.0"
