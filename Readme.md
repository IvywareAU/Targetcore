...
`CStringA::Find`/`Mid`/`Left`/`Trim`/`operator[]`, none of which `../Platform/p2pstr.h` carries. Ten
...
`../Platform/p2pthread.h` allocates a `std::recursive_mutex`, so **every startup/shutdown cycle
...
- **Two of the three substrates are substitutions, not the same code.** The IOCP pump is an io_uring
  shim (`../Platform/p2piocp.cpp`, compiled straight into the library on Linux only,
  `CMakeLists.txt:108`) and the MFC layer is `../Platform/mfcshim.h`. Crypto is a **second
...
- **`P2PeerService` is shimmed to a no-op, and that is the plan's design rather than a shortfall.**
  ...
  path becomes the Linux daemon entry — `../Platform/p2psvc.h` returns FALSE from
...
Expected layout — the two siblings are peers of this directory, not submodules:

```text
<parent>\
├── Msgcore\      ← headers + Msgcore.lib / Msgcore.dll
├── Platform\     ← platform.h, mfcshim.h, p2piocp.cpp
...
(`TargetCore(2022).vcxproj:198`, and :242, :284, :324, :393, :424, :457, :485 — one per
...
```cpp
#include "../Platform/platform.h"   // Win32: passthrough to WinSock2/ws2tcpip/mswsock/atlstr
#include "../Platform/mfcshim.h"    // Linux: CObject/CList/CMap/CString/ASSERT
```

An explicit relative path out of the project directory, on **both** platforms. Every TU includes
`stdafx.h`, so `..\Platform` is as mandatory as `..\Msgcore` — and invisible to anyone reading
only the `.vcxproj`.

**3 · Link-time and run-time — `Msgcore.lib` / `Msgcore.dll`**

`Msgcore.lib` heads the linker inputs at `TargetCore(2022).vcxproj:206` (and :248, :291, :329),
...
| `CMakeLists.txt:108` | Linux only: compiles `../Platform/p2piocp.cpp` straight into this library |
...
msbuild "TargetCore(2022).sln" /p:Configuration=Release /p:Platform=x64
```
...
`Msgcore(2022).vcxproj` has no `AdditionalIncludeDirectories` element at all, its
...
the flat C ABI — 93 symbols, enumerated and gated — is the covered surface, the C++ classes
...
retired of its VS.NET 2002 project file, carried forward to the v143 toolset and C++23, given
